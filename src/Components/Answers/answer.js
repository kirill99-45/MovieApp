...
export const Answers = ({ activeUser, comment, answer, postID, author }) => {
  ...
  const CommentConsumer = ({ consumer }) => {
    return (
      <div className='post-page__comment-consumer'>
        ...
        <Link to={ pathName(consumer.id) }>{consumer.firstName} {consumer.lastName}</Link>
      </div>
    )
  }
  ...
  const pathName = (id) => {
    if (id !== activeUser.id) {
      return `/react-first-app/users/${id}`
  ...
  return (
    ...
    <div className='comment-answer__wrapper' key={answer.id}>
      <Link to={ pathName(answer.author.id) } className='comment__author-photo' target='_blank'>
      ...
      <div className='post-page__comment-body-container'>
        <div className='post-page__comment-author-wrapper'>
          <Link to={ pathName(answer.author.id) } className='post-page__comment-author'>{answer.author.firstName} {answer.author.lastName}</Link>
          <CommentConsumer consumer={answer.consumer}/>
        </div>
